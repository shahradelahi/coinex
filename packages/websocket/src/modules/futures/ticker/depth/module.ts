...
import type {
  FuturesDepthSubscriptionParams,
  FuturesDepthUnsubscribeParams,
  FuturesDepthUpdate,
} from './types';

export class FuturesDepth {
   ...
   */
  subscribe(
    params: FuturesDepthSubscriptionParams,
    onUpdate: (update: FuturesDepthUpdate) => void
  ): number {
    const id = this.#ws.send({ method: 'depth.subscribe', params });
    this.#ws.on('message', (message: { method: string; data: FuturesDepthUpdate }) => {
   ...
   * @see https://docs.coinex.com/api/v2/futures/market/ws/market-depth
   */
  unsubscribe(params: FuturesDepthUnsubscribeParams): number {
